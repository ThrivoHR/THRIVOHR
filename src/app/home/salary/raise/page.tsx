...
import { RaiseType, SalaryFilterType, SalaryType } from "@/schemaValidation/salary.schema";
import SalaryFilter from "./filterSalary";

export default function HistoryTable() {
  const [loading, setLoading] = useState(false);
  const [raise, setRaise] = useState<RaiseType[]>([]);
  ...
  useEffect(() => {
    if (showTable) {
      const fetch = async () => {
        setLoading(true);
        try {
          const data = await apiSalaryRequest.getRaise(
            page,
            pageSize,
            filter
          );
          setRaise(data.payload.value);
        } catch (error) {
